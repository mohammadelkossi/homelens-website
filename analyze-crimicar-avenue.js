...
// Function to check if address matches Crimicar Avenue
function isCrimicarAvenue(address, street) {
  const addressLower = address?.toLowerCase() || '';
  const streetLower = street?.toLowerCase() || '';
  
  return streetLower.includes('crimicar avenue') || 
         addressLower.includes('crimicar avenue');
}
...
async function analyzeCrimicarAvenue() {
  ...
  for (const year of years) {
    ...
    try {
      ...
      for (const line of lines) {
        ...
        // Filter for S10 postcode, semi-detached, and Crimicar Avenue
        if (isS10Postcode(record.postcode) && 
            record.propertyType === 'S' && 
            isCrimicarAvenue(record.paon, record.street)) {
          crimicarSales.push(record);
        }
      }

      if (crimicarSales.length > 0) {
        console.log(`✅ ${year}: ${crimicarSales.length} semi-detached sales found on Crimicar Avenue`);
        results.push(...crimicarSales);
      } else {
        console.log(`❌ ${year}: No semi-detached sales found on Crimicar Avenue`);
      }
  ...
  console.log(`This represents ${results.length} sales over the period.`);
}

// Run the analysis
analyzeCrimicarAvenue().catch(error => {
